...
async function update(dive) {
    try {
        ...
        const set = { $set: noIdDiveObj }

        const existing = await collection.findOne(criteria)
        if (!existing) {
            throw new Error(`Dive with id ${_id} not found`)
        }

        const res = await collection.updateOne(criteria, set)

        if (res.modifiedCount === 0) console.log("No changes detected- document already have this values")
